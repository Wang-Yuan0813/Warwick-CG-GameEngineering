...
PS_OUTPUT  PS(PS_INPUT input)
{
    PS_OUTPUT output;
    //combine
    output.Colour = float4(input.Colour.xyz, 0.08);
    //output.Normal = float4(normalize(input.Normal), GetLinearDepthGray(input.Pos.z));
    output.Normal = float4(normalize(input.Normal), 0.1);
    //output.Depth = input.ViewPos.z;
    output.Depth = input.ViewPos.z;
    return output;
}*/